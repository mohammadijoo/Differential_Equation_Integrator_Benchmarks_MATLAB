...
The double pendulum is a two-link mechanical system with generalized coordinates

$$ q=[\theta_1,\theta_2]^T. $$

It can be written in manipulator form as

$$ M(q)q''+C(q,q')q'+g(q)=0. $$

### Benchmark type

This is a **nonlinear mechanical ODE** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

For the ideal frictionless system, total mechanical energy is conserved:

$$ E(q,q')=T(q,q')+V(q). $$

The motion can be chaotic and highly sensitive to initial conditions.
...
compute Coriolis/centrifugal terms C(q,dq)*dq
...
This benchmark tests nonlinear mechanical integration, sensitivity to initial conditions, and invariant drift.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_double_pendulum.m
```
...
Applications include robotics, multibody dynamics, chaos demonstrations, and mechanical simulation.
