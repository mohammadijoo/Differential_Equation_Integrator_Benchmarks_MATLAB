...
$$ x'(t)=\frac{\beta x(t-\tau)}{1+x(t-\tau)^n}-\gamma x(t). $$

### Benchmark type

This is a **nonlinear chaotic DDE** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

For sufficiently large delay, the equation can produce chaotic dynamics. A numerical reference requires careful handling of history interpolation.
...
- long-time qualitative behavior
...
This benchmark tests whether DDE solvers can reproduce chaotic delay dynamics without spurious damping or instability.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_mackey_glass_d_d_e.m
```
...
Applications include hematopoiesis modeling, delay chaos, biological regulation, and nonlinear dynamics.
