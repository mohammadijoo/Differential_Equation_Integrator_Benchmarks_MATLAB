# Ring Modulator Circuit ODE/DAE Benchmark

## Equation

A ring modulator model is often expressed as a stiff system derived from circuit laws,

$$ M y' = f(t,y), $$

where $M$ may be nonsingular for an ODE form or singular for a DAE form.

### Benchmark type

This is a **stiff circuit system** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

The benchmark usually uses a numerical reference solution. The main difficulty is the combination of fast circuit transients, nonlinear diode characteristics, and possible mass-matrix structure.

### Mathematical properties emphasized

- stiff electrical dynamics
- mass matrix
- nonlinear devices
- implicit solver workload
...
```text
compute diode currents from voltages
assemble circuit residual or right-hand side
solve M*dydt = f(t,y)
return dydt
```
...
This benchmark is useful as a bridge between stiff ODEs and circuit DAEs.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_ring_modulator_circuit_o_d_e.m
```
...
Applications include electrical circuits, communication electronics, SPICE-like simulation, and stiff solver testing.
