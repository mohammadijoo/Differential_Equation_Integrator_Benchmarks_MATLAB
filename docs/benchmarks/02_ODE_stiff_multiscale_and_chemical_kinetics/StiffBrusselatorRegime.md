...
A stiff Brusselator variant is

$$ x' = A + x^2y - (B+1)x, $$

$$ y' = Bx - x^2y, $$

with parameter choices that create fast attraction to a slow oscillatory structure.

### Benchmark type

This is a **stiff nonlinear reaction ODE** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

A high-accuracy stiff reference solution should be used. The equilibrium remains

$$ x^\ast=A,\qquad y^\ast=\frac{B}{A}. $$

### Mathematical properties emphasized

- stiff nonlinear oscillation
- limit-cycle behavior
- positivity
- Jacobian-dependent performance
...
This benchmark connects the nonstiff Brusselator with stiff reaction dynamics and helps show when explicit methods become inefficient.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_stiff_brusselator_regime.m
```
...
Applications include chemical oscillation, stiff reaction systems, and transition-to-stiffness experiments.
