# Atmospheric Pollution Stiff ODE Model

## Equation

A pollution-chemistry box model has the form

$$ y'=P(y,t)-L(y,t), $$

where $P$ and $L$ are nonlinear production and loss terms for atmospheric species.

### Benchmark type

This is a **large stiff reaction ODE** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

The model is normally treated as a stiff initial-value problem with a high-accuracy reference solution. Important diagnostics include positivity and conservation-like budgets for chemical families.

### Mathematical properties emphasized

- large stiff reaction network
- positivity
- diurnal forcing
- expensive RHS
...
```text
compute photolysis rates depending on t
compute production and loss terms for all species
return dy
```

### Main benchmark lesson

This benchmark tests whether method comparisons remain meaningful when RHS cost, Jacobian cost, and rejected steps dominate runtime.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_pollution_model_o_d_e.m
```
...
Applications include atmospheric chemistry, air-pollution simulation, climate chemistry modules, and stiff reactive transport.
