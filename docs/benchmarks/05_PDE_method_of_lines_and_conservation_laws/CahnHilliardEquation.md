...
This is a **fourth-order nonlinear PDE** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

The equation conserves mass:

$$ \frac{d}{dt}\int_\Omega u(x,t)\,dx=0 $$

and dissipates free energy under suitable boundary conditions.
...
```text
mu = -epsilon^2 * Laplacian(U) + U.^3 - U
dUdt = Laplacian(mu)
return dUdt
```
...
This benchmark stresses stiff solvers, invariant monitoring, and energy behavior in a nonlinear PDE.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_cahn_hilliard_equation.m
```
...
Applications include phase separation, materials science, diffuse-interface modeling, and gradient flows.
