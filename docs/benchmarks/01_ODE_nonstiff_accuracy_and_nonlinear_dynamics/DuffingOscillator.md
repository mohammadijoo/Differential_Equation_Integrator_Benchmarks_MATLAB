...
A common forced Duffing oscillator is
...
$$ v'=-\delta v-\alpha q-\beta q^3+\Gamma\cos(\omega t). $$

### Benchmark type

This is a **nonlinear second-order ODE** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

No general closed-form solution exists. For conservative unforced cases with $\delta=0$ and $\Gamma=0$, the energy

$$ E(q,v)=\frac{1}{2}v^2+\frac{1}{2}\alpha q^2+\frac{1}{4}\beta q^4 $$
...
- forced oscillation
- possible chaos
- energy drift in conservative limits
...
```text
dqdt = v
dvdt = -delta*v - alpha*q - beta*q^3 + Gamma*cos(omega*t)
return [dqdt; dvdt]
```

### Main benchmark lesson

This benchmark tests nonlinear oscillator accuracy, phase behavior, and robustness under forcing.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_duffing_oscillator.m
```
...
Applications include mechanical vibration, nonlinear springs, structural dynamics, MEMS, and forced nonlinear systems.
