# Robertson Problem with Algebraic Mass Conservation

## Equation

A DAE version of Robertson replaces one differential equation with the mass constraint
...
This is a **stiff semi-explicit DAE** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

The algebraic equation enforces exact mass conservation at the continuous level. A numerical solver should keep
...
```text
compute two differential reaction equations
compute algebraic residual y1 + y2 + y3 - 1
return DAE residual
```

### Main benchmark lesson

This benchmark links stiff chemical ODEs to DAE formulations where invariants are imposed algebraically.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_robertson_d_a_e.m
```
...
Applications include chemical kinetics, conservation laws, process simulation, and DAE benchmarking.
