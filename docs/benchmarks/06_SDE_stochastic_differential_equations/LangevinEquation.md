...
A simple Langevin equation is

$$ dq_t=v_t\,dt, $$

$$ m\,dv_t=-\gamma v_t\,dt-\nabla V(q_t)\,dt+\sqrt{2\gamma k_B T}\,dW_t. $$

### Benchmark type

This is a **second-order stochastic dynamics** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

For quadratic potentials, the system has known Gaussian equilibrium statistics. General potentials are judged using invariant distribution and energy/temperature diagnostics.
...
```text
dq = v*dt
dv = (-gamma*v - gradV(q))/m*dt + sqrt(2*gamma*kBT)/m*dW
```

### Main benchmark lesson

This benchmark tests stochastic mechanical integrators and long-time sampling properties.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_langevin_equation.m
```
...
Applications include molecular dynamics, statistical mechanics, Brownian motion, and stochastic simulation.
