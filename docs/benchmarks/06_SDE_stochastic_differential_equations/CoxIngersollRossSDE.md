# Cox-Ingersoll-Ross Square-Root SDE
...
This is a **positive stochastic process** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

The process is nonnegative under suitable parameter conditions. Numerical methods may produce negative values unless special care is taken.
...
```text
drift = kappa*(theta - X)
diffusion = sigma*sqrt(max(X,0))
dX = drift*dt + diffusion*dW
```

### Main benchmark lesson

This benchmark tests positivity-preserving SDE schemes and behavior near the zero boundary.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_cox_ingersoll_ross_s_d_e.m
```
...
Applications include interest-rate models, stochastic volatility, population models, and positivity-preserving SDE integration.
