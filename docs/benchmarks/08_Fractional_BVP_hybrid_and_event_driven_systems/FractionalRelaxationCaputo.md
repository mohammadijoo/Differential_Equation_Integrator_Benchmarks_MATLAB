# Fractional Caputo Relaxation Equation

## Equation

A basic Caputo fractional relaxation equation is

$$ {}^C D_t^\alpha x(t) = -\lambda x(t), \qquad 0<\alpha<1. $$

### Benchmark type

This is a **fractional differential equation** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

The exact solution is written using the Mittag-Leffler function:
...
```text
compute Caputo history convolution
advance x using fractional relaxation formula
return x_new
```

### Main benchmark lesson

This benchmark tests fractional integrators and memory-handling algorithms.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_fractional_relaxation_caputo.m
```
...
Applications include viscoelasticity, anomalous diffusion, fractional control, and memory systems.
