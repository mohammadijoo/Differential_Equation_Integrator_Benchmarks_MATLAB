# Bratu Boundary-Value Problem

## Equation

The Bratu problem is

$$ y''+\lambda e^y=0,\qquad 0<x<1, $$
...
This is a **nonlinear boundary-value problem** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

The problem has parameter-dependent solution branches. It is a standard test for nonlinear BVP solvers and continuation methods.
...
```text
convert to first-order system y1'=y2
y2'=-lambda*exp(y1)
enforce boundary residuals y1(0)=0 and y1(1)=0
```

### Main benchmark lesson

This benchmark tests collocation, shooting, finite-difference BVP methods, and nonlinear solver robustness.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_bratu_b_v_p.m
```
...
Applications include combustion theory, nonlinear elliptic equations, and BVP solver validation.
