...
This is a **third-order nonlinear BVP** benchmark proposed for future simulations in this repository.

### Reference solution or diagnostic quantity

The semi-infinite boundary condition is approximated by truncating the domain at a large finite value. Shooting methods solve for the unknown initial value $f''(0)$.

### Mathematical properties emphasized

- third-order nonlinear BVP
...
y2'=y3
y3'=-0.5*y1*y3
adjust y3(0) so y2(L)=1
```

### Main benchmark lesson

This benchmark tests shooting, collocation, and sensitivity of BVP solvers to unknown initial slopes.

## Proposed MATLAB file

```text
src/benchmarks/benchmark_blasius_boundary_layer.m
```
...
Applications include fluid mechanics, boundary layers, aerodynamics, and BVP methods.
