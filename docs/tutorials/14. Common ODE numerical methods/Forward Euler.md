...
A first-order explicit one-step method using the derivative at the current point.
...
Algorithm sketch for using or implementing Forward Euler:
...
```matlab
% Forward Euler for x' = -x
h = 0.1; t = 0:h:5; x = zeros(size(t)); x(1)=1;
for n = 1:numel(t)-1
    x(n+1) = x(n) + h*(-x(n));
end
