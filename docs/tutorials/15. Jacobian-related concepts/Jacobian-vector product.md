...
**Section:** 15. Jacobian-related concepts

## 1. Meaning

The product of a Jacobian with a vector, often used without forming the full Jacobian.
...
For a vector field $f:\mathbb{R}^n\to\mathbb{R}^n$, the Jacobian is
...
The Jacobian controls local linear behavior, Newton iterations, stiffness detection, and implicit solver efficiency.
...
Conceptual workflow for Jacobian-vector product:
...
```matlab
% Jacobian and eigenvalue example
f = @(x) [x(1)^2 + x(2); sin(x(1)) + x(2)^3];
x = [1; 2];
J_analytical = [2*x(1), 1; cos(x(1)), 3*x(2)^2]
lambda = eig(J_analytical)
