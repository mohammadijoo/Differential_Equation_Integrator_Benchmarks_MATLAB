...
A boundary condition requiring the solution and often derivatives to match across opposite boundaries.
...
For a time-dependent PDE, one may specify

$$u(x,0)=u_0(x),\qquad u(0,t)=a(t),\qquad u(L,t)=b(t).$$

The first is an initial condition, while the second and third are boundary conditions.
...
ODE IVPs need initial values, while PDEs often need both initial values and boundary behavior at the domain edges.
...
Conceptual workflow for Periodic boundary condition:
